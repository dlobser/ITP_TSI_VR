...
public class SampleParametric : MonoBehaviour
{
    ...
    void Build()
    {
        switch (whichFunction)
        {
            ...
            case 4:
                ...
                break;
            case 5:
                GetComponent<MeshFilter>().mesh = Grid.Generate(uDivs, vDivs, Cylinder);
                break;
            case 6:
                GetComponent<MeshFilter>().mesh = Grid.Generate(uDivs, vDivs, Cone);
                break;
            case 7:
                GetComponent<MeshFilter>().mesh = Grid.Generate(uDivs, vDivs, PlaneTall);
                break;
            default:
                break
            ;
    ...
    {
        return new Vector3(u - .5f, v, 0);
    }

    Vector3 PlaneTall(float u, float v)
    {
        return new Vector3(u * uDivs, v * vDivs, 0);
    }

    Vector3 Sphere(float u, float v)
    {
        float X = Mathf.Sin(u * Mathf.PI ) * Mathf.Sin(v * Mathf.PI)*-1;
        float Y = Mathf.Cos(v * Mathf.PI);
        float Z = Mathf.Cos(u * Mathf.PI ) * Mathf.Sin(v * Mathf.PI);
        return new Vector3(X, Y, Z);
    }

    Vector3 Cylinder(float u, float v)
    {
        float X = Mathf.Sin(u * Mathf.PI * -2);//+ Mathf.Abs(Mathf.Sin(u*Mathf.PI*6))*.1f;// * Mathf.Sin(v * Mathf.PI);
        // float Y = Mathf.Cos(v * Mathf.PI);
        float Z = Mathf.Cos(u * Mathf.PI * 2);//+ Mathf.Abs(Mathf.Cos(u*Mathf.PI*6))*.1f;// * Mathf.Sin(v * Mathf.PI);
        return new Vector3(X, Mathf.Pow(v, 3) * 50, Z);
    }

    Vector3 Cone(float u, float v)
    {
        float X = Mathf.Sin(u * Mathf.PI * 2);//+ Mathf.Abs(Mathf.Sin(u*Mathf.PI*6))*.1f;// * Mathf.Sin(v * Mathf.PI);
        // float Y = Mathf.Cos(v * Mathf.PI);
        float Z = Mathf.Cos(u * Mathf.PI * -2);//+ Mathf.Abs(Mathf.Cos(u*Mathf.PI*6))*.1f;// * Mathf.Sin(v * Mathf.PI);
        return new Vector3(X * v, v, Z * v);
    }
